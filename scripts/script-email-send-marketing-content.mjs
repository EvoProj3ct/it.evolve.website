import { parseOptionalEventIdArgs } from "./lib/args.mjs";
...
const SCRIPT_NAME = "script-email-send-marketing-content";
const COLLECTION_NAME = "consultation_requests";

async function main() {
  const { eventId } = parseOptionalEventIdArgs();
  const filters = {
    ...(eventId ? { eventId } : {}),
    "consents.privacyAccepted": true,
    "consents.dataSecurityAccepted": true,
    "consents.productsServicesAccepted": true,
  };
  ...
  const filename = eventId
    ? `email-send-marketing-content-${safeFilenamePart(eventId)}.json`
