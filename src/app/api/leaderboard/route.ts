...
const toPairs = (raw: any): Array<{ member: string; score: number }> => {
    ...
    if (raw.length && Array.isArray(raw[0])) {
        return raw
            ...
            .filter((p: any) => typeof p.member === "string" && p.member.length > 0 && Number.isFinite(p.score));
    }
...
export async function GET(req: NextRequest) {
    ...
    // Batch meta (HMGET if supported; fallback to Promise.all)
    let metas: Array<string | null> = [];
    try {
        // @ts-expect-error optional wrapper feature
        metas = members.length ? await kv.hmget<string>(META_KEY, ...members) : [];
    } catch {
        metas = await Promise.all(members.map((m) => kv.hget<string>(META_KEY, m)));
    }
